...
| `user_input_request` | `requestId`, `questions`, `autoResolutionMs?` | Provider-native questions that pause the current turn. Option descriptions are optional display metadata. |
| `user_input_resolved` | `requestId`, `resolution`, `answeredQuestionIds`, `answers?` | Resolves the matching native question without creating a new user turn. Secret values are omitted. |
...
- **`user_input_request`** — render every provider-native question and answer it through the host's dedicated reply channel while the current turn remains active. Render an option's `description` when present and omit the description row when absent. Support single choice, multi-select, free text/Other, and secret inputs according to each question's flags.
- **`user_input_resolved`** — replace the matching request's pending state. `resolution: "auto"` means the provider continued without a submitted answer. `answers` may include non-secret values for replay; use `answeredQuestionIds` to mark secret questions complete without revealing their values.
...
`onUserInputRequest` is intentionally not part of `bridge.callbacks`: the application owns that async request/response lifecycle. It should emit `user_input_request`, keep the runner callback pending until its dedicated answer channel resolves, then emit `user_input_resolved`. The answer resumes the active provider turn and is not a new user message.
